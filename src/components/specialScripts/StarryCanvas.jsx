    ...
    useEffect(() => {
        ...
        class ShootingStar {
            constructor(x, y, angle, speed) {
                ...
                this.color = ['139,129,255', '252,185,0', '252,237,214'][Math.floor(Math.random() * 3)];
            }

            draw() {
                // Gradient for the trail
                const gradientTrail = ctx.createLinearGradient(this.x - this.length * this.dx, this.y - this.length * this.dy, this.x, this.y);
                ...
                ctx.beginPath();
                ctx.moveTo(this.x - this.length * this.dx, this.y - this.length * this.dy);
                ctx.lineTo(this.x, this.y);
                ctx.stroke();

                // Gradient for the head
                const gradientHead = ctx.createRadialGradient(this.x, this.y, 0, this.x, this.y, ctx.lineWidth + 0.5);
